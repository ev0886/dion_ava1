...
class RealRfidAdapter(RealHardwareAdapterBase):
    ...
    def _parse_read_response(cls, response: str) -> str:
        ...
        if not lines:
            return response
        return lines[-1] if lines[-1] != cls._READ_RESPONSE else cls._READ_RESPONSE
    ...
    def _read_card_via_sdk_transport(self) -> RfidReadResult:
        ...
        uid = result.uid
        uid_size = result.uid_size
        if uid_size < 0:
            raise HardwareFailureError(
                f"RFID reader returned an invalid UID size: {uid_size}.",
                device_type=self.device_type,
                operation="read_card",
            )

        if result.status_type == 1 or uid_size == 0:
            self._last_uid = None
            return RfidReadResult(
                device_type=self.device_type,
                status=HardwareOperationStatus.NO_CARD,
                ...
                uid=None,
                is_duplicate=False,
                message=f"No valid RFID card present ({DEFAULT_RFID_UID_FORMAT})",
            )

        if uid is None:
            raise HardwareFailureError(
                "RFID reader returned a contradictory SDK card-read response: missing UID with non-NO_CARD status.",
                device_type=self.device_type,
                operation="read_card",
            )
    ...
    def _require_rfid_config(self, *, operation: str) -> HardwareEndpointTransportConfig:
        self._raise_if_unavailable(operation=operation)
        config = self._config
        if config is None:
            raise HardwareUnavailableError(
