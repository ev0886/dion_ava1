(function () {
  ...
  function renderCountdown() {
    ...
    elements.idleReturnChip.textContent = "Reset in " + Math.ceil(remainingMs / 1000) + " s";
  }
  ...
  function setResult(kind, title, detail, operation) {
    ...
    elements.resultPanel.classList.add(kind === "success" ? "result-success" : kind === "warning" ? "result-warning" : kind === "failure" ? "result-failure" : "result-neutral");
    elements.resultStatePill.textContent = kind === "success" ? "Success" : kind === "warning" ? "Recovery" : kind === "failure" ? "Attention" : "Idle";
    ...
    elements.confirmedQty.textContent = operation && operation.qty_confirmed != null ? String(operation.qty_confirmed) : "n/a";
  ...
  function renderSelectedOption() {
    elements.dispenseTarget.textContent = state.selectedOption ? state.selectedOption.item_name + " | qty " + state.selectedOption.total_quantity : "Select an item";
    syncDispenseButton();
  }

  function selectOption(optionKey) {
    state.selectedOption = state.availableOptions.find((option) => String(option.item_id) === optionKey) || state.availableOptions[0] || null;
    elements.dispenseOptions.querySelectorAll(".option-button").forEach((button) => {
      button.classList.toggle("option-selected", button.dataset.optionKey === String(state.selectedOption ? state.selectedOption.item_id : ""));
    });
  ...
  function renderOptions(options) {
    ...
    elements.inventoryQuantity.textContent = String(state.availableOptions.reduce((sum, option) => sum + (option.total_quantity || 0), 0));
    ...
    state.availableOptions.forEach((option) => {
      ...
      button.dataset.optionKey = String(option.item_id);
      button.innerHTML = '<span class="option-title">' + option.item_name + '</span><span class="option-meta">Qty ' + option.total_quantity + "</span>";
      button.addEventListener("click", function () {
        selectOption(String(option.item_id));
      });
      elements.dispenseOptions.appendChild(button);
    });
    selectOption(String((state.selectedOption || state.availableOptions[0]).item_id));
  }
  ...
  function classifyOperation(operation) {
    if (!operation || !operation.operation_state) {
      return { kind: "failure", title: "Invalid operation result", detail: "Backend did not return an operation state." };
    }
    if (operation.operation_state === "completed") {
      return { kind: "success", title: "Dispense complete", detail: "Inventory was updated and dispense completed successfully." };
    }
    if (operation.operation_state === "recovery_required") {
      return { kind: "warning", title: "Recovery required", detail: operation.error_message || "Recovery is required before further work." };
    }
    return { kind: "failure", title: "Dispense failed", detail: operation.error_message || operation.result || "Operation finished with an error." };
  }
  ...
  async function startDispense() {
    ...
    try {
      ...
      setMachineState("Result", result.detail, result.kind === "success" ? "success" : result.kind === "warning" ? "warning" : "danger");
